...
import cn.moretv.doraemon.common.alg.{Algorithm, AlgParameters, Model}
...
/**
  ...
  */
class LongVideoClusterAlgorithm  extends Algorithm{
  override protected val algParameters: LongVideoClusterParameters = new LongVideoClusterParameters()
  ...
  override protected def invoke(): Unit = {
    ...
    //1.获取用户电影聚类的特征 RDD[(uid, Array[(cluster_Index, Double, Double)])]
    ...
    val recommendLongVideos = processedUserFeatures.map(e => {
      ...
      e._2.foreach(x => {
        result = ArrayUtils.arrayAlternate(result, cluster2longVideoMap.getOrElse(x, Array.empty[String]), 1, 1)
      })
      (uid, result)
    }).
      filter(e => e._2.length > 0).
