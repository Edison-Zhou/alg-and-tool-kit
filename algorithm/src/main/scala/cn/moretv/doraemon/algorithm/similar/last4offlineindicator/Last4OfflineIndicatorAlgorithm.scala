...
/**
  ...
  */
class Last4OfflineIndicatorAlgorithm extends Algorithm{
  override protected val algParameters: Last4OfflineIndicatorParameter = new Last4OfflineIndicatorParameter()
  override protected val modelOutput: Last4OfflineIndicatorModel = new Last4OfflineIndicatorModel()

  //数据Map的Key定义
  //用户观看的最后一部电影
  val INPUT_USER_WATCHED_LAST_MOVIES = "userWatchedLastMovie"
  //相似影片数据
  val INPUT_SIMILAR_MOVIE = "similarMovies"
  //用户近期看过的电影
  ...
  override protected def invoke(): Unit = {
    ...
    /**
      * ---------------------------------------------获取数据，并转换---------------------------------------------
      */
    //1.获取n天前开始计时的前两个月内用户观看的最后一部电影 RDD[uid, (sid, score)]
    val endDateImpression = DateUtils.farthestDayWithDelimiter(algParameters.nDaysAgoEnd)
    val startDateImpression = DateUtils.farthestDayWithDelimiter(algParameters.nDaysAgoEnd+algParameters.numOfDaysRetainAnImpression)
    val userWatchedLastMovies=dataInput(INPUT_USER_WATCHED_LAST_MOVIES)
      ...
      groupByKey().map(e => (e._1, e._2.toArray.maxBy(x => x._1))).map(e => (e._1, (e._2._2, e._2._3)))
    ...
    //3.获取用户15天前及更早看过的电影 RDD[(uid, Array[sid])]
    val endDateWatched = DateUtils.farthestDayWithDelimiter(algParameters.nDaysAgoEnd)
